...
const fetchAPI = async ( endpoint: string, method: string = 'GET', body?: any, isFormData = false ) => {
  ...
  const res = await fetch(`${API_URL}${formattedEndpoint}`, {
    method,
    headers,
...
export const login = async (email: string, password: string) => {
  const res = await fetch(`${API_URL}/auth/login/`, { // Added /api/auth/login/
    method: 'POST',
